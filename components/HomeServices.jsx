      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full mb-5"
            ...
            <span className="text-sm font-semibold" style={{ color: "var(--accent)" }}>What We Do</span>
          </div>
          ...
          <p className="text-base max-w-2xl mx-auto" style={{ color: "var(--muted)" }}>
            From design to deployment — we cover every aspect of your digital journey with precision and passion.
          </p>
        </motion.div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12" style={{ perspective: "1200px" }}>
          {displayed.map((service, i) => (
            <motion.div key={service.id}
              ...
              style={{ background: "#ffffff", border: `1.5px solid ${service.accent}30` }}
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-3xl font-black opacity-15 select-none" style={{ color: service.accent }}>{service.number}</span>
              </div>
              <h3 className="text-lg font-bold mb-1" style={{ color: "#111" }}>{service.title}</h3>
              <p className="text-xs font-semibold mb-2" style={{ color: service.accent }}>{service.subtitle}</p>
              <p className="text-sm leading-relaxed mb-4 line-clamp-2 flex-1" style={{ color: "#555" }}>{service.desc}</p>
              ...
              <button onClick={(e) => { e.stopPropagation(); router.push(`/services/${service.id}`); }}
        ...
        <div className="text-center">
          <motion.button whileHover={{ scale: 1.05, boxShadow: "0 0 32px rgba(45,77,202,0.4)" }} whileTap={{ scale: 0.97 }}
            onClick={() => { router.push("/services"); window.scrollTo({ top: 0 }); }}
