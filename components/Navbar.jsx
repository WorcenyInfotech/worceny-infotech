...
export default function Navbar() {
  ...
  useEffect(() => {
    if (pathname === "/contact") setActiveLink("Contact");
    else if (pathname === "/portfolio") setActiveLink("Portfolio");
    else if (pathname.startsWith("/services")) setActiveLink("Services");
    else if (pathname.startsWith("/industries")) setActiveLink("Industries");
    else if (pathname.startsWith("/technologies")) setActiveLink("Technologies");
  ...
  useEffect(() => {
    const onDoc = (e) => {
      if (!navRef.current?.contains(e.target)) setMegaOpen(null);
    };
  ...
  const clearMegaLeave = () => {
    if (megaLeaveTimer.current) { clearTimeout(megaLeaveTimer.current); megaLeaveTimer.current = null; }
  };
  ...
  const megaLabel = (key) =>
    key === "services" ? "Services" : key === "industries" ? "Industries" : "Technologies";

  const megaPath =
    megaOpen === "services" ? "/services" : megaOpen === "industries" ? "/industries" : "/technologies";
      ...
      <div className="relative" onMouseLeave={scheduleMegaClose}>
        <div className="max-w-7xl mx-auto px-6 sm:px-8 py-4 flex items-center justify-between gap-6">
          <motion.div
            ...
            onMouseEnter={() => { clearMegaLeave(); setMegaOpen(null); }}
            onClick={() => handleNav({ path: "/", section: "home" })}
          >
            <picture>
              <source media="(max-width: 767px)" srcSet="/logo-mobile.png" />
              <img src="/logo.png" alt="Worceny Infotech" className="h-8 object-contain" />
            </picture>
          </motion.div>
          ...
          <ul className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            {megaKeys.map((key) => {
              ...
              return (
                <li key={key} className="relative" onMouseEnter={() => { clearMegaLeave(); setMegaOpen(key); }}>
                  <button
                    type="button"
                    className="relative py-1 text-sm font-medium group cursor-pointer flex items-center gap-1"
                    onClick={() => goMega(key === "services" ? "/services" : key === "industries" ? "/industries" : "/technologies")}
                    aria-expanded={open}
                    aria-haspopup="true"
                  >
                    <span
                      className="transition-colors duration-300"
                      style={{ color: isActive || open ? "var(--accent)" : "var(--muted)" }}
                      onMouseEnter={(e) => { if (!isActive && !open) e.target.style.color = "var(--text)"; }}
                      onMouseLeave={(e) => { if (!isActive && !open) e.target.style.color = "var(--muted)"; }}
                    >
                      {label}
                    </span>
                    <FiChevronDown
                    ...
                    <motion.span
                      className="absolute -bottom-1 left-0 right-0 h-px rounded-full"
                      style={{ background: "var(--accent)" }}
                      animate={{ opacity: isActive || open ? 1 : 0, scaleX: isActive || open ? 1 : 0 }}
                      transition={{ duration: 0.3 }}
                    />
                  </button>
                </li>
            ...
            >
              <div className="mx-auto max-w-7xl">
                <div
                  ...
                  style={{ background: "#ffffff", borderColor: "rgba(45,77,202,0.12)", boxShadow: "0 24px 60px rgba(15,23,42,0.12)" }}
                >
                  <div className="p-6 lg:p-8">
                    {megaOpen === "services" && (
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                        {services.map((s) => (
                          <button key={s.id} type="button" onClick={() => goMega(`/services/${s.id}`)}
                            ...
                            <div className="text-xs mt-0.5 line-clamp-2" style={{ color: "var(--muted-card)" }}>{s.subtitle}</div>
                          </button>
                        ))}
                      </div>
                    )}
                    {megaOpen === "industries" && (
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[min(70vh,420px)] overflow-y-auto pr-1">
                        {industries.map((ind) => (
                          <button key={ind.id} type="button" onClick={() => goMega(`/industries/${ind.id}`)}
                            className="text-left rounded-xl px-3 py-2.5 transition-colors hover:bg-[rgba(45,77,202,0.06)] cursor-pointer">
                            <div className="text-sm font-semibold" style={{ color: "var(--text)" }}>{ind.title}</div>
                            <div className="text-xs mt-0.5 line-clamp-2" style={{ color: "var(--muted-card)" }}>{ind.subtitle}</div>
                          </button>
                        ))}
                      </div>
                    )}
                      ...
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 max-h-[min(72vh,520px)] overflow-y-auto pr-1">
                        {techGroups.map((g) => (
                          <div key={g.id} className="min-w-0">
                            <button type="button" onClick={() => goMega(`/technologies/${g.id}`)}
                              className="text-left w-full rounded-lg px-1 py-1 mb-2 transition-colors hover:bg-[rgba(40,77,202,0.08)] cursor-pointer">
                              <div className="text-sm font-black leading-tight" style={{ color: "var(--text)" }}>{g.label}</div>
                              <div className="text-[11px] mt-0.5 line-clamp-2" style={{ color: "var(--muted-card)" }}>{g.subtitle}</div>
                            </button>
                            <ul className="space-y-0.5 border-t pt-2" style={{ borderColor: "rgba(45,77,202,0.1)" }}>
                              {g.techs.map((t) => (
                                <li key={`${g.id}-${t.slug}`}>
                                  <button type="button" onClick={() => goMega(`/technologies/${g.id}/${t.slug}`)}
                    ...
                    <div className="mt-5 pt-4 flex justify-end border-t" style={{ borderColor: "rgba(45,77,202,0.1)" }}>
                      <button type="button" onClick={() => goMega(megaPath)}
                        className="text-sm font-bold cursor-pointer" style={{ color: "var(--accent)" }}>
          ...
          >
            <ul className="flex flex-col px-6 py-5 gap-1">
              {simpleLinks.map((link, i) => (
                <motion.li key={link.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                  <button onClick={() => handleNav(link)}
                    ...
                    style={{ color: activeLink === link.label ? "var(--accent)" : "rgba(255,255,255,0.72)", background: activeLink === link.label ? "var(--surface)" : "transparent" }}>
                    {link.label}
                  </button>
                </motion.li>
              ))}
              ...
              {megaKeys.map((key, mi) => {
                ...
                const basePath = key === "services" ? "/services" : key === "industries" ? "/industries" : "/technologies";
                const items = key === "services" ? services : key === "industries" ? industries : techGroups;
                return (
                  <motion.li key={key} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: (simpleLinks.length + mi) * 0.06 }}
                    className="rounded-lg overflow-hidden"
                    style={{ background: expanded ? "rgba(255,255,255,0.06)" : "transparent" }}>
                    <div className="flex items-center">
                      <button onClick={() => goMega(basePath)}
                        className="flex-1 text-left py-2.5 px-3 text-sm font-medium cursor-pointer"
                        style={{ color: activeLink === label && !expanded ? "var(--accent)" : "rgba(255,255,255,0.9)" }}>
                        {label}
                      </button>
                      <button type="button" aria-expanded={expanded} className="p-2.5 pr-3 cursor-pointer"
                        onClick={() => setMobileMega(expanded ? null : key)}
                        style={{ color: "rgba(255,255,255,0.7)" }}>
                        <FiChevronRight size={18} className={`transition-transform ${expanded ? "rotate-90" : ""}`} />
                      </button>
                    </div>
                    <AnimatePresence>
                      {expanded && (
                        <motion.ul initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                          exit={{ height: 0, opacity: 0 }} className="pl-3 pb-2 space-y-1 overflow-hidden">
                          {key === "technologies"
                            ? techGroups.flatMap((g) => [
                                <li key={`h-${g.id}`} className="pt-1 pb-0.5">
                                  <span className="px-3 text-[10px] font-bold uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.45)" }}>{g.label}</span>
                                </li>,
                                ...g.techs.map((t) => (
                                  <li key={`${g.id}-${t.slug}`}>
                                    <button type="button" onClick={() => goMega(`${basePath}/${g.id}/${t.slug}`)}
                                      className="w-full text-left py-1.5 px-3 rounded-md text-xs cursor-pointer"
                                      style={{ color: "rgba(255,255,255,0.8)" }}>{t.name}</button>
                                  </li>
                                )),
                                <li key={`a-${g.id}`} className="pb-1">
                                  <button type="button" onClick={() => goMega(`${basePath}/${g.id}`)}
                                    className="w-full text-left py-1.5 px-3 rounded-md text-[11px] font-semibold cursor-pointer"
                                    style={{ color: "var(--accent)" }}>{g.label} — overview →</button>
                                </li>,
                              ])
                            : items.map((item) => {
                                const id = item.id;
                                ...
                                return (
                                  <li key={id}>
                                    <button type="button" onClick={() => goMega(`${basePath}/${id}`)}
                                      className="w-full text-left py-2 px-3 rounded-md text-xs cursor-pointer"
                                      style={{ color: "rgba(255,255,255,0.75)" }}>{title}</button>
                                  </li>
                                );
                              })}
                        </motion.ul>
              ...
              {[{ label: "Portfolio", path: "/portfolio" }, { label: "Contact", path: "/contact" }].map((link, i) => (
                <motion.li key={link.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: (simpleLinks.length + megaKeys.length + i) * 0.06 }}>
                  <button onClick={() => handleNav(link)}
                    className="w-full text-left py-2.5 px-3 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer"
                    style={{ color: activeLink === link.label ? "var(--accent)" : "rgba(255,255,255,0.72)", background: activeLink === link.label ? "var(--surface)" : "transparent" }}>
                    {link.label}
              ...
              <motion.li initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: (simpleLinks.length + megaKeys.length + 2) * 0.06 }}>
                <button onClick={() => { router.push("/contact"); window.scrollTo({ top: 0 }); setMenuOpen(false); }}
