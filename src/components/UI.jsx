...
export function CardHover({ children, accent = 'var(--accent)', className = '' }) {
  return (
    <motion.div
...
export function AccentButton({ children, onClick, secondary = false }) {
  return (
    <motion.button
      whileHover={{ scale: 1.05, boxShadow: secondary ? 'none' : '0 0 28px rgba(134,90,255,0.5)' }}
      whileTap={{ scale: 0.95 }}
      onClick={onClick}
      className="px-7 py-3 rounded-full font-bold text-sm transition-all duration-300"
      style={secondary
        ? { background: 'rgba(134,90,255,0.1)', border: '1px solid rgba(134,90,255,0.28)', color: 'var(--accent)' }
