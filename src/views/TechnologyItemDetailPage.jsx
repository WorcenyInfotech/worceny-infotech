...
export default function TechnologyItemDetailPage() {
  const { id, techSlug } = useParams();
  const router = useRouter();
  const pageRef = useRef();

  const result = id && techSlug ? getTechItem(id, techSlug) : null;
  const { prev, next } =
    id && techSlug
      ? getTechSiblings(id, techSlug)
      : { prev: null, next: null };
          ...
          >
            {prev ? (
              <motion.button
                ...
                type="button"
                onClick={() => {
                  router.push(`/technologies/${group.id}/${prev.slug}`);
                  window.scrollTo({ top: 0 });
                }}
            ...
            {next ? (
              <motion.button
                ...
                type="button"
                onClick={() => {
                  router.push(`/technologies/${group.id}/${next.slug}`);
                  window.scrollTo({ top: 0 });
                }}
