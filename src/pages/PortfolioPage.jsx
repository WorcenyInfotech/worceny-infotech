...
const projects = [
  { id: 1, title: 'E-Commerce Platform',    category: 'Full Stack', year: '2024',
    ...
    accent: '#865aff', gradient: 'rgba(134,90,255,0.15), rgba(90,255,115,0.08)',
  ...
  { id: 2, title: 'SaaS Analytics Dashboard', category: 'Frontend', year: '2024',
    ...
    accent: '#5aff73', gradient: 'rgba(90,255,115,0.15), rgba(134,90,255,0.08)',
  ...
  { id: 3, title: 'Corporate Website',       category: 'Web Dev',    year: '2023',
    ...
    accent: '#865aff', gradient: 'rgba(134,90,255,0.15), rgba(90,255,115,0.06)',
        ...
        >
          ...
          >
            ...
            <div className="absolute top-3 right-3 px-3 py-1 rounded-full text-xs"
              style={{ background: 'rgba(10,10,11,0.6)', color: 'var(--muted)' }}>
              {project.year}
            </div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}>
            ...
            <h3 className="text-2xl font-bold mb-3" style={{ color: 'var(--text)' }}>{project.title}</h3>
            <p className="leading-relaxed mb-6 text-sm" style={{ color: 'var(--muted)' }}>{project.desc}</p>

            {/* Features */}
            <div className="mb-6">
              <h4 className="text-sm font-semibold mb-3" style={{ color: 'var(--text)' }}>Key Features</h4>
    ...
    <div ref={sectionRef} className="min-h-screen pt-24" style={{ background: 'var(--bg)' }}>
      ...
      <div className="relative py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          <div className="port-hero-text inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full mb-6"
            ...
            <span className="text-sm font-semibold" style={{ color: 'var(--accent)' }}>Our Work Speaks for Itself</span>
          </div>
          <h1 className="port-hero-text text-5xl md:text-7xl font-black mb-6" style={{ color: 'var(--text)' }}>
            Featured <span className="gradient-text">Projects</span>
          </h1>
          <p className="port-hero-text text-lg max-w-2xl mx-auto leading-relaxed" style={{ color: 'var(--muted)' }}>
            A curated showcase of our best work — from startups to enterprise solutions, built with passion and precision.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
        ...
        <div className="stats-row grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {stats.map(s => (
            <motion.div key={s.label}
              whileHover={{ scale: 1.05, boxShadow: '0 0 28px rgba(134,90,255,0.18)' }}
              className="stat-item rounded-2xl p-6 text-center"
              style={{ background: 'var(--card-bg)', border: '1px solid rgba(134,90,255,0.18)' }}>
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden"
                  ...
                  <motion.div whileHover={{ scale: 1.1 }} transition={{ duration: 0.5 }}
                    ...
                    <span className="text-8xl font-black" style={{ color: p.accent, opacity: 0.15 }}>{p.title[0]}</span>
                  </motion.div>
                  <div className="absolute top-3 left-3 px-3 py-1 rounded-full text-xs"
                    style={{ background: 'rgba(10,10,11,0.65)', color: 'var(--muted)' }}>{p.year}</div>
                  {/* Hover overlay */}
                  <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} transition={{ duration: 0.28 }}
                    className="absolute inset-0 flex items-center justify-center"
                    style={{ background: 'rgba(10,10,11,0.7)' }}>
                    <span className="flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold"
                      style={{ border: `1px solid ${p.accent}`, color: p.accent }}>
                      View Details <FiArrowRight size={14} />
                    </span>
                  </motion.div>
                  {/* Shimmer */}
                  <motion.div initial={{ x: '-100%' }} whileHover={{ x: '200%' }} transition={{ duration: 0.65 }}
                ...
                <div className="p-6">
                  ...
                  <p className="text-sm line-clamp-2 mb-4" style={{ color: 'var(--muted)' }}>{p.desc}</p>
        ...
        >
          ...
          <p className="mb-8 max-w-xl mx-auto text-sm" style={{ color: 'var(--muted)' }}>
