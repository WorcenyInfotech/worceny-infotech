...
export default function TechnologyItemDetailPage() {
  ...
  const { prev, next } =
    groupId && techSlug ? getTechSiblings(groupId, techSlug) : { prev: null, next: null }
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        '.ti-hero-el',
        { opacity: 0, y: 40 },
        { opacity: 1, y: 0, duration: 0.85, stagger: 0.08, ease: 'power3.out', delay: 0.05 },
      )
      gsap.fromTo(
        '.ti-hl',
        { opacity: 0, x: -12 },
    ...
    <div ref={pageRef} className="min-h-screen pt-16" style={{ background: 'var(--bg)' }}>
      <div className="relative py-16 md:py-20 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          ...
          <div className="ti-hero-el inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6"
            style={{ background: `${group.accent}14`, border: `1px solid ${group.accent}35` }}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-8 mb-10">
            ...
            >
              <img src={tech.icon} alt="" className="w-14 h-14 object-contain" />
            </motion.div>
            <div>
              <h1 className="ti-hero-el text-4xl md:text-5xl font-black mb-3 leading-tight" style={{ color: 'var(--text)' }}>
                {tech.name}
              </h1>
              <p className="ti-hero-el text-lg font-medium mb-4" style={{ color: group.accent }}>
                {tech.desc}
              </p>
            </div>
          </div>
          ...
          <div className="ti-highlights mb-16">
            <h2 className="text-sm font-bold uppercase tracking-widest mb-4" style={{ color: group.accent }}>
              Highlights
            </h2>
            <div className="grid gap-3">
              {tech.highlights.map((h) => (
                <motion.div
                  ...
                  style={{ background: 'var(--card-bg)', border: `1px solid ${group.accent}18` }}
                >
                  <span
                  ...
                  <span className="text-sm font-medium pt-1.5" style={{ color: 'var(--text-card)' }}>
                    {h}
                  </span>
                </motion.div>
              ))}
          ...
          <div className="flex flex-wrap gap-4 mb-16">
            <motion.button
              whileHover={{ scale: 1.04, boxShadow: `0 0 28px ${group.accent}45` }}
              whileTap={{ scale: 0.97 }}
              type="button"
              onClick={() => {
                navigate('/contact')
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between pb-16 border-t pt-10"
            ...
            {prev ? (
              <motion.button
                ...
                style={{ background: 'var(--card-bg)', border: `1px solid ${group.accent}22`, color: 'var(--text-card)' }}
              >
                <FiArrowLeft size={16} style={{ color: group.accent }} />
                <div>
                  <div className="text-xs mb-0.5" style={{ color: 'var(--muted-card)' }}>
