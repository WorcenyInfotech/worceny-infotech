  ...
  if (!industry) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: 'var(--bg)' }}>
        <h2 className="text-3xl font-black mb-4" style={{ color: 'var(--text)' }}>Industry Not Found</h2>
  ...
  return (
    <div ref={pageRef} className="min-h-screen pt-16" style={{ background: 'var(--bg)' }}>
      ...
      <div className="relative py-20 overflow-hidden">
        ...
        <motion.div animate={{ scale: [1, 1.25, 1], opacity: [0.08, 0.16, 0.08] }}
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <motion.button
            ...
            onMouseEnter={e => e.currentTarget.style.color = industry.accent}
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* Left */}
            <div>
              <div className="id-hero-el inline-flex items-center gap-2.5 px-4 py-2 rounded-full mb-6"
                style={{ background: `${industry.accent}15`, border: `1px solid ${industry.accent}35` }}>
                <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: industry.accent }} />
                <span className="text-sm font-semibold" style={{ color: industry.accent }}>Industry</span>
              </div>
              <h1 className="id-hero-el text-4xl md:text-6xl font-black mb-4 leading-tight" style={{ color: 'var(--text)' }}>
                {industry.title}
              </h1>
              <p className="id-hero-el text-xl font-medium mb-6" style={{ color: industry.accent }}>
                {industry.subtitle}
              </p>
              <p className="id-hero-el text-base leading-relaxed mb-8" style={{ color: 'var(--muted)' }}>
                {industry.desc}
              </p>
              ...
              <div className="id-hero-el flex flex-wrap gap-2 mb-8">
                {industry.clients.map(c => (
                  <span key={c} className="text-xs px-3 py-1.5 rounded-full font-medium"
                    style={{ background: `${industry.accent}12`, color: industry.accent, border: `1px solid ${industry.accent}30` }}>
                    {c}
                  </span>
                ))}
              </div>
              ...
              <div className="id-hero-el flex flex-wrap gap-4">
                <motion.button
                  whileHover={{ scale: 1.05, boxShadow: `0 0 32px ${industry.accent}55` }}
                  whileTap={{ scale: 0.96 }}
                ...
                <motion.button
                  ...
                  style={{ background: 'transparent', border: `1.5px solid ${industry.accent}50`, color: 'var(--text)' }}>
                  All Industries
                </motion.button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center">
              <motion.div
                ...
                <motion.div animate={{ rotate: -360 }} transition={{ duration: 14, repeat: Infinity, ease: 'linear' }}
                  ...
                  style={{ top: '32px', left: '32px', border: `1px dashed ${industry.accent}50` }} />
                <div className="w-64 h-64 rounded-full flex items-center justify-center relative z-10"
                  style={{ background: `radial-gradient(circle, ${industry.accent}20, ${industry.accent}05)`, border: `2px solid ${industry.accent}25` }}>
                  <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 3, repeat: Infinity }}
                    className="w-28 h-28 rounded-3xl flex items-center justify-center"
                    style={{ background: `${industry.accent}20`, color: industry.accent, fontSize: '3.5rem' }}>
                    {industry.iconLg}
                  </motion.div>
                </div>
                {/* Floating client badge */}
                <div className="absolute -top-4 -right-4 px-3 py-2 rounded-2xl text-xs font-bold text-center"
                  style={{ background: industry.accent, color: '#fff', maxWidth: 80 }}>
                  {industry.clients[0]}
                </div>
              </motion.div>
            </div>
        ...
        <div className="id-stats grid grid-cols-2 md:grid-cols-4 gap-4">
          {industry.stats.map((s, i) => (
            <motion.div key={i}
              ...
              style={{ background: 'var(--card-bg)', border: `1px solid ${industry.accent}20` }}>
              <div className="text-3xl font-black mb-1" style={{ color: industry.accent }}>{s.v}</div>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6"
              style={{ background: `${industry.accent}12`, border: `1px solid ${industry.accent}25` }}>
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: industry.accent }}>Overview</span>
            </div>
          ...
          <div className="id-solutions">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6"
              style={{ background: `${industry.accent}12`, border: `1px solid ${industry.accent}25` }}>
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: industry.accent }}>Our Solutions</span>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {industry.solutions.map((sol, i) => (
                <motion.div key={sol}
                  whileHover={{ x: 6, boxShadow: `0 0 20px ${industry.accent}15` }}
                  transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                  className="id-solution flex items-center gap-4 p-4 rounded-2xl"
                  style={{ background: 'var(--card-bg)', border: `1px solid ${industry.accent}15` }}>
                  <span className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
                    style={{ background: `${industry.accent}20`, color: industry.accent }}>
                    <FiCheck size={14} />
                  </span>
                  <span className="text-sm font-medium" style={{ color: 'var(--text-card)' }}>{sol}</span>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mb-20">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4"
            ...
            <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: industry.accent }} />
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: industry.accent }}>Key Features</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
            {industry.features.map((feat, i) => (
              <motion.div key={feat}
                ...
                whileHover={{ y: -5, boxShadow: `0 10px 28px ${industry.accent}20`, borderColor: `${industry.accent}50` }}
                className="id-feature flex items-center gap-3 p-4 rounded-2xl transition-all duration-300"
                style={{ background: '#f8f7ff', border: `1px solid ${industry.accent}18` }}>
                <span className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
                  style={{ background: `${industry.accent}18`, color: industry.accent }}>
                  <FiCheck size={13} />
                </span>
                <span className="text-sm font-semibold" style={{ color: '#111' }}>{feat}</span>
              </motion.div>
            ))}
          </div>

          {/* Bottom bar */}
          <div className="w-full h-px rounded-full mt-8"
            style={{ background: `linear-gradient(90deg, transparent, ${industry.accent}40)` }} />
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mb-20">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4"
            style={{ background: `${industry.accent}12`, border: `1px solid ${industry.accent}25` }}>
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: industry.accent }}>Tech Stack</span>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {industry.techStack.map((tech, i) => (
            <motion.div key={tech}
              ...
              whileHover={{ y: -4, boxShadow: `0 8px 24px ${industry.accent}30` }}
              className="px-6 py-3 rounded-2xl text-sm font-bold cursor-default"
              style={{ background: '#ffffff', border: `1px solid ${industry.accent}30`, color: '#111' }}>
              {tech}
            </motion.div>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mb-20">
        <motion.div
          ...
          style={{ background: 'var(--card-bg)', border: `1px solid ${industry.accent}25` }}>
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: `radial-gradient(circle at 50% 0%, ${industry.accent}12, transparent 60%)` }} />
          <h2 className="text-3xl md:text-4xl font-black mb-4 relative z-10" style={{ color: 'var(--text-card)' }}>
            Ready to Build for <span className="gradient-text">{industry.title}?</span>
          </h2>
          ...
          <div className="flex flex-wrap gap-4 justify-center relative z-10">
            <motion.button
              whileHover={{ scale: 1.06, boxShadow: `0 0 36px ${industry.accent}55` }}
              whileTap={{ scale: 0.95 }}
              onClick={() => { navigate('/contact'); window.scrollTo({ top: 0 }) }}
            ...
            <motion.button
              ...
              onClick={() => navigate('/industries')}
              className="px-10 py-4 rounded-full font-bold text-base cursor-pointer"
              style={{ background: 'transparent', border: `1.5px solid ${industry.accent}40`, color: 'var(--text-card)' }}>
              All Industries
            </motion.button>
          </div>
        </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          {prev ? (
            <motion.button whileHover={{ x: -4 }} whileTap={{ scale: 0.97 }}
              onClick={() => { navigate(`/industries/${prev.id}`); window.scrollTo({ top: 0 }) }}
          ...
          {next ? (
            <motion.button whileHover={{ x: 4 }} whileTap={{ scale: 0.97 }}
              onClick={() => { navigate(`/industries/${next.id}`); window.scrollTo({ top: 0 }) }}
              className="flex items-center gap-3 px-6 py-4 rounded-2xl text-sm font-semibold cursor-pointer"
